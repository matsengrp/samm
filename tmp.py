...
nest = SConsWrap(Nest(base_dict=base), '_output', alias_environment=env)
...
@nest.add_target_with_env(env)
def generate(env, outdir, c):
    cmd = 'python simulate_from_sampled_gls.py simulate --seed $SOURCE --output_file ${TARGETS[0]} --output_genes ${TARGETS[1]}'
    return env.Command(
        [join(outdir, 'seqs.csv'), join(outdir, 'genes.csv')],
        c['seed'],
        cmd)
...
#        [c['generate'][0], c['generate'][1], c['estimation_method']],
#        cmd)
